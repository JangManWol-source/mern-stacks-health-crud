...
const Drawer = () => {
  ...
  return (
    <div className='w-full text-white absolute bg-gray-800'>
        <Link to={'/add/'} onClick={()=> {
            dispatch(toggleDrawer())
        }}>
        <div className='p-1 text-sm font-bold hover:text-yellow-600'>Create New Data</div>
        </Link>

        <Link to={'/'} onClick={()=> {
            dispatch(toggleDrawer())
        }}>
        <div className='p-1 text-sm font-bold hover:text-yellow-600'>Home</div>
        </Link>
        <div className="w-full flex justify-center p-2 border-t-2 bg-gray-900 ">
